class Validation {
    ...
    numberCheck(input) {
        if (!input.val().match(/\d/)) { return "Debe contener un número. "; } else { return ""; }
    }
    specialCharCheck(input) {
        if (!input.val().match(/\W|_/g)) { return "Debe contener un caracter especial. "; } else { return ""; }
    }
    capitalCheck(input) {
        if (input.val().match(/[A-Z]+/)) { return ""; } else { return "Debe contener mayúsculas. "; }
    }
    passwordMatchCheck(input, passConfirm) {
        if (input.val() === passConfirm.val()) { return ""; } else { return "Las contraseñas no coinciden. "; }
    }
    emailCheck(input) {
        if (input.val().match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) { return ""; } else { return "No es un e-mail válido"; }
    }
